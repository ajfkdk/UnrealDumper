...
struct Offsets {
  ...
  struct {
    ...
    uint16 HeaderSize =
        2; // Size of FNameEntry header (offset where a string begins)
  } FNameEntry;
  struct {
    uint16 Index = 0xc; // Offset to index of this object in all objects array
    uint16 Class = 0x10; // Offset to UClass pointer (UClass* ClassPrivate)
    uint16 Name = 0x18;  // Offset to FName structure
    uint16 Outer = 0x20; // (UObject* OuterPrivate)
  } UObject;
  struct {
    uint16 Next = 0x28;
  } UField;
  ...
  struct {
    ...
    uint16 Func = 0xB0+0x28; // ue3-ue4, always +0x28 from flags location.
  } UFunction;
  struct {
    uint16 Class = 0x8;
    uint16 Next = 0x20;
